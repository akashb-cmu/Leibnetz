...
def flatten_mnist_image(image_mat):
    image_mat = np.array(image_mat)
    return(image_mat.flatten())

def flatten_mnist_images(images_list):
    ret_images = []
    for image in images_list:
        ret_images.append(flatten_mnist_image(image))
    return(np.array(ret_images))

def binarize_labels(labels, dig_pair):
    assert len(dig_pair) == 2, "No of labels inappropriate"
    ret_labels = []
    dig1 = dig_pair[0]
    for label in labels:
        if label == dig1:
            ...
            ret_labels.append(1)
    return np.array(ret_labels, dtype=np.int32)

def get_one_hot(vec_size, label):
    vec = np.zeros(vec_size, dtype=np.int32)
    vec[label] = 1
    return(vec)

def get_multiclass_labels(labels, n_classes):
    ret_labels = []
    for label in labels:
        ret_labels.append(get_one_hot(vec_size=n_classes, label=label))
    return(np.array(ret_labels))

def get_binarized_accuracy(predicted_labels, gold_labels):
    assert len(predicted_labels) == len(gold_labels), "Label sizes unequal"
    return(np.sum(predicted_labels==gold_labels) * 100.0 / len(gold_labels))

def get_multiclass_accuracy(predicted_labels, gold_labels):
    assert len(predicted_labels) == len(gold_labels), "Label sizes don't match"
    nr = np.sum(np.argmax(predicted_labels, axis=1)==np.argmax(gold_labels, axis=1))
    dr = len(gold_labels)
    return(np.sum(np.argmax(predicted_labels, axis=1)==np.argmax(gold_labels, axis=1)) * 100.0 / len(gold_labels))
