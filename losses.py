...
def mean_squared_logarithmic_loss(y_pred, y_actual, **kwargs):
    ...
    eprint("Using mean squared loss. Ensure all outputs (pred and actual) are >= 0")
...
def squared_hinge_loss(y_pred, y_actual, **kwargs):
    ...
    eprint("Using squared hinge loss. Ensure labels are +1, -1 and NOT 0.")
...
def hinge_loss(y_pred, y_actual, **kwargs):
    # Used for training classifiers. Assumes the true labels are +1 and -1 (and NOT 0)
    eprint("Using hinge loss. Ensure labels are +1 -1 and not 0.")
...
def sparse_categorical_crossentropy_loss(y_pred, y_actual, from_logits=False, **kwargs):
    ...
    """
    eprint("Using sparse categorical cross-entropy. Ensure y_actual is a vector with the appropriate integer class "
...
def binary_crossentropy_loss(y_pred, y_actual, from_logits=False, **kwargs):
    ...
    """
    eprint("Binary cross entropy expects gold labels that are 1 or 0 and model "
...
def get_loss(loss_type, y_pred, y_actual, **kwargs):
    ...
    eprint("Not all losses are averaged across samples! This could lead to varying sensitivities to learning rates!")
